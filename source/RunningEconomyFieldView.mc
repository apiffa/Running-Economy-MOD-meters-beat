...
class RunningEconomyFieldView extends Ui.SimpleDataField {
    ...
    function initialize() {
        ...
        var mProfile 		= UserProfile.getProfile();
 		...
 		mEconomyField 		 = createField("running_economy", 0, FitContributor.DATA_TYPE_FLOAT, { :mesgType=>FitContributor.MESG_TYPE_RECORD });
        mAverageEconomyField = createField("average_economy", 1, FitContributor.DATA_TYPE_FLOAT, { :mesgType=>FitContributor.MESG_TYPE_SESSION });
        mLapEconomyField 	 = createField("lap_economy", 	  2, FitContributor.DATA_TYPE_FLOAT, { :mesgType=>FitContributor.MESG_TYPE_LAP });
    ...
    function compute(info) {
    	if (mTimerRunning) {  //! We only do calculations if the timer is running (distance isn't accumulated otherwise)
			...
			//! Running economy from IMGrant see Copyright below modified for giving meters/beat instead
			//! Alvaro Piffaretti apiffa@gmail.com April 2017
			//! Copyright and intelectual property as original released by IMGrant 
			...
			if (mLastNElapsedDistance > 0) {
				...
				mLastNEconomy = ( (mLastNElapsedDistance) / (mLastNAvgHeartRate * t) );
			}
			
			mLastNEconomySmooth = (0.222222 * mLastNEconomy) + (0.777777 * mLastNEconomySmooth);
			
	        mEconomyField.setData(mLastNEconomySmooth.toFloat());

	        if (mAverageHeartRate > uRestingHeartRate && mElapsedDistance > 0) {
	        	mAverageEconomy = ( (mElapsedDistance) / ((mAverageHeartRate) * (info.timerTime / 60000.0)) );
	        }

	        mAverageEconomyField.setData(mAverageEconomy.toFloat());

	        if (mLapHeartRate > uRestingHeartRate && mLapElapsedDistance > 0) {
	        	mLapEconomy = ( (mLapElapsedDistance) / ( mLapHeartRate * (mLapTimerTime / 60000.0)) );
	        }
	        mLapEconomyField.setData(mLapEconomy.toFloat());
	        
	        mPrevElapsedDistance = mElapsedDistance;
    	}	

        return mLastNEconomySmooth.toFloat();
    }
